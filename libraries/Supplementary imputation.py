...
def imputation_linear(Y_nm, Y_m, Y_input):
    reg = LinearRegression()
    reg.fit(Y_nm, Y_m)
    Y_output = reg.predict(Y_input)
    return Y_output
def imputation_SVR(Y_nm, Y_m, Y_input):
    Y_output = []
    ...
    return Y_output
def imputation_GPR(Y_nm, Y_m, Y_input):
    gp = GaussianProcessRegressor(n_restarts_optimizer=5, random_state = 10)
    gp.fit(Y_nm, Y_m)
    Y_output = gp.predict(Y_input)
    return Y_output
def imputation_cGAN1(Y_nm, Y_m, Y_val_nm, Y_val_m, Y_input, seed_sampling, latent_dim = 1, epochs = 1000):
    ...
    return Y_output
def imputation_CVAE0(Y_nm, Y_m, Y_val_nm, Y_val_m, Y_input, seed_sampling, latent_dim = 1, epochs = 1000): #imputation by linear regression
    ...
    class Decoder(nn.Module):
        def __init__(self, input_dim, output_dim):
            ...
            self.decoder_logits = nn.Linear(input_dim, output_dim)
        def forward(self, x):
            x_hat = self.decoder_logits(x)
            return x_hat
    ...
    torch.manual_seed(seed_sampling)
    DEVICE = torch.device("cpu")
    encoder = Encoder(input_dim=class_dim + target_dim, output_dim=latent_dim)
    decoder = Decoder(input_dim=class_dim + latent_dim, output_dim=target_dim)
    ...
    return Y_output
def imputation_CVAE1(Y_nm, Y_m, Y_val_nm, Y_val_m, Y_input, seed_sampling, latent_dim = 1, epochs = 1000):
    class Encoder(nn.Module):
        def __init__(self, input_dim, output_dim, hidden_dim):
            super(Encoder, self).__init__()
