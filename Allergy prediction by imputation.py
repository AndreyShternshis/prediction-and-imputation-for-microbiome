...
for i in np.unique(Individual_ID):
    ...
    if np.mean(Outcomes)==Outcomes[-1]: #considering data points with unique tp
        ...
        if np.sum(TP == 0) == 1 and np.sum(TP == 1) == 1 and np.sum(TP == 2) == 0:
        ...
        if np.sum(TP == 0) == 1 and np.sum(TP == 1) == 0 and np.sum(TP == 2) == 1:
        ...
        if np.sum(TP == 0) == 0 and np.sum(TP == 1) == 1 and np.sum(TP == 2) == 1:
...
for IS in range(Imputation_sets):
    ...
    for TS in range(test_sets): #test sets
        ...
        for VS in range(val_sets): #val sets
            ...
            X_train, classes_train, label_train = np.concatenate([X_train, X_01_cut, X_02_cut, X_12_cut]), np.concatenate([classes_train, classes_01, classes_02, classes_12]), np.concatenate([label_train, labels_01, labels_02, labels_12])
        ...
        X_rest, label_rest, classes_rest = np.concatenate([X_train, X_val]), np.concatenate([label_train,label_val]), np.concatenate([classes_train,classes_val])#concatenate everything except testing set
