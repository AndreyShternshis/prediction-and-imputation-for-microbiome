...
###parameters
test = 0.2 #fractiion for testing
...
X_full, X_02, X_12, X_2, labels, labels_02, labels_12 = [[] for _ in range(7)] #X is data in 3 rows, labels are EPDS at tp 2
...
for TS in range(test_sets): #test sets
    ...
    for IS in range(imputation_sets):
        print("imputation group", IS)
        seed_sampling = ITER + IS
        for VS in range(val_sets): #val sets
            ...
            class_12, class_02 = np.zeros((np.shape(X_12_cut)[0],2)), np.zeros((np.shape(X_02_cut)[0],2))
            class_12[:,1], class_02[:,0] = 1, 1
            ###imputation
            X_12_cut = Imputation_library.impute_one(X_train,X_val,X_12_cut, d, [1,2], 0, seed_sampling)
            X_02_cut = Imputation_library.impute_one(X_train, X_val, X_02_cut, d, [0,2], 1, seed_sampling)
            X_train, label_train, class_train = np.concatenate([X_train,X_12_cut,X_02_cut]), np.concatenate([label_train,labels_12,labels_02]), np.concatenate([class_train,class_12,class_02]) #concatenate imputed sets to training set
            X_train, X_val = X_train[:,:2,:], X_val[:,:2,:]
            X_train, X_val = np.reshape(X_train, (-1, dY)) / 2, np.reshape(X_val, (-1, dY)) / 2 #flatten inputs to one compositional vector
        ...
        X_test_cut = X_test_cut[:,:2,:]
        X_test_cut = np.reshape(X_test_cut, (-1, dY)) / 2 #flatten testing set
...
print("true positive",np.mean(TRUE_POSITIVE))
print("true negative",np.mean(TRUE_NEGATIVE))
